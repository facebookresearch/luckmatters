...
if __name__ == '__main__':
    ...
    if num_process == 1:
        ...
        for folder in folders:
            entry = process_one(folder)
            all_results.append(entry)
            
    else:
        import multiprocessing as mp
        pool = mp.Pool(num_process)
        chunksize = (len(folders) + num_process - 1) // num_process

        results = pool.imap_unordered(process_one, folders, chunksize=chunksize)

        all_results = []
        for entry in tqdm.tqdm(results, total=len(folders)):
            all_results.append(entry)
        
    torch.save(all_results, "results.pth")
